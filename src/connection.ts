...
switch (action) {
    ...
    //Add a Role
        ...
        ]).then(({ title, salary, departmentId }) => addRole(title, salary, departmentId));
    ...
    case 'Add a Employee':
        ...
        ]).then(({ employeeId, roleId }) => updateEmployeeRole(employeeId, roleId));
      ...
      return inquirer.prompt([
       ...
       ]).then (({ employeeId, managerId }) => updateEmployeeManager(employeeId, managerId));
